...
def _get_settings(agent):  # type: ignore
    model = agent.model
...
def test_factories_carry_model_settings():  # type: ignore
    settings = get_settings()

    extraction_agent = create_extraction_agent(settings)
    ms = _get_settings(extraction_agent)  # type: ignore
    assert ms is not None
    assert "openrouter_reasoning" in ms or "openai_reasoning_effort" in ms

    analysis_agent = create_analysis_agent(settings)
    assert _get_settings(analysis_agent) is not None  # type: ignore

    alias_agent = create_alias_resolution_agent(settings)
    assert _get_settings(alias_agent) is not None  # type: ignore

    summary_agent = create_summarization_agent(settings)
    assert _get_settings(summary_agent) is not None  # type: ignore


def test_unprefixed_model_string():  # type: ignore
    settings = get_settings()
    ...
    ms = _get_settings(agent)  # type: ignore
    assert ms is not None
